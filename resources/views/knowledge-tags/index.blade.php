<x-app-layout>
        ...
        <div class="w-full max-w-none mx-auto px-4 sm:px-6 lg:px-8 xl:px-10 2xl:px-12 space-y-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 border-b border-gray-200">
                    <form method="GET"
                          action="{{ route('knowledge-tags.index') }}"
                          id="knowledge-tags-filter-form"
                        ...
                        <div class="flex items-end gap-2">
                            ...
                            <a href="{{ route('knowledge-tags.index') }}"
                ...
                <form method="POST" action="{{ route('knowledge-tags.bulk-save') }}" id="knowledge-tags-form">
                    @csrf
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Tag Name</th>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Tag Type</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($rows as $row)
                                    <tr>
                                        <td class="px-3 py-2">
                                            <input type="text"
                                                   name="existing[{{ $row->id }}][tagname]"
                                                   value="{{ old("existing.{$row->id}.tagname", $row->tagname) }}"
                                        ...
                                        <td class="px-3 py-2">
                                            <select name="existing[{{ $row->id }}][tagtype]"
                                                    class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                                                <option value="">None</option>
                                                @foreach($tagTypeOptions as $value => $label)
                                                    <option value="{{ $value }}"
                                                        @selected(old("existing.{$row->id}.tagtype", $row->tagtype) === $value)>
                                                        {{ $label }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <td class="px-3 py-2 text-center whitespace-nowrap">
                                            <button type="button"
                                                    class="px-3 py-1.5 bg-red-100 text-red-700 rounded hover:bg-red-200 text-sm delete-knowledge-tag-btn"
                                                    data-id="{{ $row->id }}"
                                                    data-name="{{ $row->tagname }}"
                                                    data-action="{{ route('knowledge-tags.destroy', $row->id) }}">
                                                Delete
                                            </button>
                                        </td>
                                    </tr>
                                ...
                                <tr class="bg-blue-50">
                                    ...
                                    <td class="px-3 py-2">
                                        <select name="new[tagtype]"
                                                class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                                            <option value="">None</option>
                                            @foreach($tagTypeOptions as $value => $label)
                                                <option value="{{ $value }}" @selected(old('new.tagtype') === $value)>
                                                    {{ $label }}
                                                </option>
                                            @endforeach
                                        </select>
                    ...
                    <div class="p-6 border-t border-gray-200 flex items-center justify-between">
                        <p class="text-sm text-gray-500">
                            Edit rows above, add a new knowledge tag at the bottom, then save once.
                        </p>
                ...
                @include('partials.admin.compact-delete-form', [
                    'formId' => 'delete-knowledge-tag-form',
    ...
    @include('partials.admin.dirty-form-script', [
        'formId' => 'knowledge-tags-form',
        'filterFormId' => 'knowledge-tags-filter-form',
        'deleteFormId' => 'delete-knowledge-tag-form',
        'deleteButtonSelector' => '.delete-knowledge-tag-btn',
        'dirtyMessage' => 'You have unsaved changes in the Knowledge Tags table. Continue and lose those changes?',
        'deleteDirtyMessage' => 'You have unsaved changes in the Knowledge Tags table. Delete anyway and lose those changes?',
        'deleteConfirmPrefix' => 'Delete knowledge tag',
